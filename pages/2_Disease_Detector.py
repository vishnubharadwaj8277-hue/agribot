...
def predict_image(model, img):
    try:
        ...
        # The model returns two outputs: [class_probs, severity_output]
        predictions = model.predict(img_array)
        class_probs = predictions[0]
        severity_output = predictions[1] # Keep this line to correctly unpack model output
        ...
        if disease == "Healthy Plant":
            scale = 0.0
        # Return both values, the caller will decide what to display
    ...
    with st.spinner(t("Validating image...", lang)):
        if not is_crop_image(img):
            ...
            with st.spinner(t("Analyzing...", lang)):
                disease, scale = predict_image(model, img) # Capture scale, but won't display it

            # Use one column or a two-column layout where the second is empty
            ...
            with col2:
                # Severity display code REMOVED HERE to not make it visible.
                # Adding a blank, equally-sized container to maintain layout balance.
                st.markdown(f"""
